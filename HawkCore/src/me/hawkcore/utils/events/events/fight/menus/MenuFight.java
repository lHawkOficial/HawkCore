...
public class MenuFight extends Menu implements MenuExecutor, MenuInterface {
	...
	public MenuFight(String title, int row, List<String> glass) {
		...
		this.iconTop = MenuAPI.getItemFromSection(Fight.get().getConfig().getConfigurationSection("MenuMain.Icons.iconTop"));
		register();
	}
	...
	public void open(Player p) {
		try {
			...
			replaces.add("{rank}:"+Fight.get().getRanking().getRankingPlayer(p.getName()));
			replaces.add("{total}:"+Fight.get().getRanking().getWinsPlayer(p.getName()));
	...
	public static MenuFight get() {
		return Fight.get().getMenu();
	}
}
