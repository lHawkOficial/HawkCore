...
@Getter
@Setter
public class MenuParkour extends Menu implements MenuExecutor, MenuInterface {
	...
	public MenuParkour(String title, int row, List<String> glass) {
		super(title, row, glass);
		this.menutop = new MenuTop(Parkour.get().getConfig().getString("MenuTop.title").replace("&", "§"), Parkour.get().getConfig().getInt("MenuTop.row"), Parkour.get().getConfig().getStringList("MenuTop.Glass"));
		this.iconBack = MenuAPI.getItemFromSection(Parkour.get().getConfig().getConfigurationSection("MenuMain.Icons.iconBack"));
		this.iconInfoPlayer = MenuAPI.getItemFromSection(Parkour.get().getConfig().getConfigurationSection("MenuMain.Icons.iconInfoPlayer"));
		this.iconInfoEvent = MenuAPI.getItemFromSection(Parkour.get().getConfig().getConfigurationSection("MenuMain.Icons.iconInfoEvent"));
		this.iconTop = MenuAPI.getItemFromSection(Parkour.get().getConfig().getConfigurationSection("MenuMain.Icons.iconTop"));
		register();
	}
	...
	public void open(Player p) {
		try {
			...
			replaces.add("{rank}:"+Parkour.get().getRanking().getRankingPlayer(p.getName()));
			replaces.add("{total}:"+Parkour.get().getRanking().getWinsPlayer(p.getName()));
	...
	public static MenuParkour get() {
		return Parkour.get().getMenu();
	}
}
