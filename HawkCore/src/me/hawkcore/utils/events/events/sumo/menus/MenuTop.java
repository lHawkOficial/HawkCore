...
public class MenuTop extends Menu implements MenuExecutor, MenuInterface {
	...
	public MenuTop(String title, int row, List<String> glass) {
		...
		iconBack = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuTop.Icons.iconBack"));
		iconPlayers = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuTop.Icons.iconPlayers"));
		iconNonPlayer = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuTop.Icons.iconNonPlayer"));
		this.slots = new ArrayList<>();
		for(int slot : Sumo.get().getConfig().getIntegerList("MenuTop.slots")) {
	...
	public void open(Player p) {
		try {
			...
			RankingEvent ranking = Sumo.get().getRanking();
