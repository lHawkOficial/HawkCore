package me.hawkcore.utils.events.events.sumo.menus;
...
@Getter
@Setter
public class MenuSumo extends Menu implements MenuExecutor, MenuInterface {
	...
	public MenuSumo(String title, int row, List<String> glass) {
		super(title, row, glass);
		this.menutop = new MenuTop(Sumo.get().getConfig().getString("MenuTop.title").replace("&", "§"), Sumo.get().getConfig().getInt("MenuTop.row"), Sumo.get().getConfig().getStringList("MenuTop.Glass"));
		this.menukit = new MenuKit("§8Visualizar Kit", 9*5, new ArrayList<>());
		this.iconBack = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuMain.Icons.iconBack"));
		this.iconInfoPlayer = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuMain.Icons.iconInfoPlayer"));
		this.iconInfoEvent = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuMain.Icons.iconInfoEvent"));
		this.iconTop = MenuAPI.getItemFromSection(Sumo.get().getConfig().getConfigurationSection("MenuMain.Icons.iconTop"));
		register();
	}
	...
	public void open(Player p) {
		try {
			...
			replaces.add("{rank}:"+Sumo.get().getRanking().getRankingPlayer(p.getName()));
			replaces.add("{total}:"+Sumo.get().getRanking().getWinsPlayer(p.getName()));
	...
	public static MenuSumo get() {
		return Sumo.get().getMenu();
	}
}
