...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public void setBordas(ItemStack item) {
		List<Block> blocos = getBordasTOP();
		for(Block b : blocos) {
	...
	public void setBiome(Biome bioma) {
		World world = Bukkit.getWorld(worldName);
		for(Block b : this.getBlocks()) {
			Location loc = b.getLocation().clone();
			loc.setY(world.getMaxHeight());
			Block bloco = world.getHighestBlockAt(loc);
			loc = bloco.getLocation().clone();
			while(loc.getY() > 0) {
				loc.getBlock().setBiome(bioma);
	...
	public List<Location> getLocations() {
		...
		getBlocks().forEach(b -> locs.add(b.getLocation()));
		return locs;
	}
	
	public List<Location> getBordas() {
		List<Location> locs = new ArrayList<>();
		getFace(CuboidDirection.East).forEach(b ->{
			locs.add(b.getLocation());
		});
		getFace(CuboidDirection.West).forEach(b ->{
			locs.add(b.getLocation());
		});
		getFace(CuboidDirection.North).forEach(b ->{
			locs.add(b.getLocation());
		});
		getFace(CuboidDirection.South).forEach(b ->{
			locs.add(b.getLocation());
		});
		return locs;
	}
	
	public List<Block> getBordasTOP() {
		List<Block> blocos = new ArrayList<>();
		World world = Bukkit.getWorld(worldName);
		getFace(CuboidDirection.East).forEach(b ->{
			Location l = b.getLocation().clone();
			l.setY(world.getMaxHeight());
			Block bloco = l.getBlock();
			Location loc = bloco.getLocation().clone();
			while (loc.getBlock().getType() != Material.GRASS && loc.getY() > 0) {
				loc = loc.clone().add(0, -1, 0);
			}
			if (loc.getBlock().getType() == Material.GRASS)
				blocos.add(loc.getBlock().getLocation().clone().add(0,1,0).getBlock());
		});
		getFace(CuboidDirection.West).forEach(b ->{
			Location l = b.getLocation().clone();
			l.setY(world.getMaxHeight());
	...
	public List<Chunk> getChunks() {
		...
		ListIterator<Block> it = getBlocks().listIterator();
		while(it.hasNext()) {
			Block b = it.next();
